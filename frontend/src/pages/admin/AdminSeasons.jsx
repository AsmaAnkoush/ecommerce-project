...
import { getAdminSeasons, createSeason, updateSeason, deleteSeason, toggleSeasonVisibility } from '../../api/seasonApi'
...
const SEASON_KEY_OPTIONS = [
  { value: '', label: '— None —' },
  { value: 'SUMMER', label: 'SUMMER' },
  { value: 'WINTER', label: 'WINTER' },
  { value: 'ALL_SEASON', label: 'ALL_SEASON' },
]
...
export default function AdminSeasons() {
  ...
  const [form, setForm] = useState({ name: '', description: '', seasonKey: '' })
  ...
  const fetchSeasons = () =>
    getAdminSeasons().then(r => setSeasons(r.data.data ?? [])).finally(() => setLoading(false))
  useEffect(() => { fetchSeasons() }, [])

  const handleToggleVisibility = async (season) => {
    const id = season.id
    const previous = !!season.visible
    const optimistic = !previous
    setSeasons(prev => prev.map(s => s.id === id ? { ...s, visible: optimistic } : s))
    setTogglingId(id)
    try {
      const res = await toggleSeasonVisibility(id)
      const next = res?.data?.data?.visible ?? optimistic
      setSeasons(prev => prev.map(s => s.id === id ? { ...s, visible: next } : s))
      toast(next ? t('admin.seasonNowVisible') : t('admin.seasonNowHidden'))
    } catch (err) {
      setSeasons(prev => prev.map(s => s.id === id ? { ...s, visible: previous } : s))
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (form.seasonKey) formData.append('seasonKey', form.seasonKey)
      if (imageFile) formData.append('image', imageFile)

      const wasEdit = !!editingId
      if (wasEdit) await updateSeason(editingId, formData)
      else await createSeason(formData)
      ...
      if (wasEdit) {
        ...
        toast(t('admin.seasonAddedSuccess'))
        setSuccessMessage(t('admin.seasonAddedSuccess'))
  ...
  const resetForm = () => {
    setForm({ name: '', description: '', seasonKey: '' })
    setImageFile(null)
    setPreviewUrl('')
    setEditingId(null)
  ...
  const startEdit = s => {
    setEditingId(s.id)
    setForm({ name: s.name, description: s.description || '', seasonKey: s.seasonKey || '' })
    setImageFile(null)
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await deleteSeason(confirmTarget.id)
      setSeasons(prev => prev.filter(s => s.id !== confirmTarget.id))
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={t('admin.headerSeasonsSub')}
        icon="🌸"
        color="#7B1E2B"
      />
      <div className="p-8 max-w-4xl pt-0 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <h2 className="font-semibold text-gray-900 mb-4">{editingId ? t('admin.editSeason') : t('admin.addSeason')}</h2>
          {successMessage && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={e => setForm(f => ({ ...f, seasonKey: e.target.value }))}
                className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#6B1F2A] focus:outline-none focus:ring-1 focus:ring-[#6B1F2A]"
              >
                {SEASON_KEY_OPTIONS.map(o => (
                  <option key={o.value} value={o.value}>{o.label}</option>
                ))}
              </select>
              <p className="mt-1 text-xs text-gray-400">{t('admin.seasonKeyHint')}</p>
            </div>
            ...
            <div className="flex gap-2">
              <Button type="submit" loading={saving}>{editingId ? t('admin.update') : t('admin.addSeason')}</Button>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-900">{t('admin.allSeasons')} ({seasons?.length ?? 0})</h2>
          </div>
          {loading ? (
                  ...
                  <div className="flex items-center gap-3">
                    {s.imageUrl ? (
                      <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                        <img src={s.imageUrl} alt={s.name} className="w-full h-full object-cover cursor-pointer"
                             onClick={() => setLightboxSrc(s.imageUrl)} />
                      </div>
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className={`font-medium text-sm truncate ${s.visible ? 'text-gray-900' : 'text-gray-500'}`}>{s.name}</p>
                        {s.seasonKey && (
                          <span
                            className="inline-flex items-center text-[10px] font-semibold px-2 py-0.5 rounded-full border tabular-nums"
                            style={{ borderColor: '#EDD8DC', background: '#FDF0F2', color: '#6B1F2A' }}
                          >
                            {s.seasonKey}
                          </span>
                        )}
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      ...
                      <button
                        ...
                        title={s.visible ? t('admin.seasonVisible') : t('admin.seasonHidden')}
                        aria-label={s.visible ? t('admin.seasonVisible') : t('admin.seasonHidden')}
                        ...
                        onMouseEnter={e => e.currentTarget.style.background = s.visible ? '#D1FAE5' : '#E5E7EB'}
                        onMouseLeave={e => e.currentTarget.style.background = s.visible ? '#ECFDF5' : '#F3F4F6'}
                      >
                        {togglingId === s.id ? (
                      ...
                      {/* View products (only when seasonKey is set) */}
                      {s.seasonKey && (
                        <Link
                          to={`/products?season=${s.seasonKey}`}
                          target="_blank"
