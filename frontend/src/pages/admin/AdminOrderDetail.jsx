    ...
    <div className="p-6 max-w-4xl">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            ...
            {order.isGuest && <span className="ml-2 text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Guest</span>}
          </p>
        </div>
      ...
      {order.status !== 'CANCELLED' && (
        <div className="bg-white rounded-2xl shadow-sm p-5 mb-4">
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">Order Progress</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div key={item.id} className="flex gap-4 px-5 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-1.5">
                      {item.size && (
                        <span className="inline-flex items-center gap-1 text-[11px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md">
                          <span className="text-gray-400 text-[10px]">SIZE</span> {item.size}
                        </span>
                      )}
                      {item.color && (
                        <span className="inline-flex items-center gap-1.5 text-[11px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md">
                          <span className="text-gray-400 text-[10px]">COLOR</span> {item.color}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <p className="font-bold text-gray-900 text-sm">₪{Number(item.subtotal).toFixed(0)}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-4">
          ...
          {/* Shipping */}
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Shipping</h2>
            ...
            {order.trackingNumber && (
              <p className="text-xs text-gray-400 mt-2">Tracking: <span className="font-medium text-gray-700">{order.trackingNumber}</span></p>
            )}
          </div>
          ...
          {order.notes && (
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5">
              <h2 className="text-xs font-semibold text-amber-600 uppercase tracking-widest mb-2">Customer Notes</h2>
          ...
          {order.paymentMethod && (
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-2">Payment</h2>
          ...
          {/* Status update */}
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3">Update Status</h2>
            <select
              value={order.status}
            ...
            {statusSuccess && (
              <p className="text-xs text-green-600 mt-2 font-medium">Status updated successfully!</p>
            )}
          </div>
        </div>
      </div>
