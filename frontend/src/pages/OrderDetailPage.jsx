    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="bg-green-50 border border-green-200 rounded-2xl p-5 mb-8 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-green-700 mt-0.5">
              Your order has been placed successfully. We will contact you via WhatsApp to confirm your order and arrange payment and delivery.
            </p>
          </div>
        </div>
      )}
      ...
      {order.status !== 'CANCELLED' && (
        <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6 mb-6">
          <h2 className="text-sm font-semibold text-gray-700 mb-5 uppercase tracking-wide">Order Progress</h2>
              ...
              {STATUS_STEPS.map((step, i) => (
                <div key={step} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-[9px] sm:text-xs text-gray-500 mt-1 w-12 sm:w-16 text-center leading-tight">{step}</p>
                  </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <h2 className="text-sm font-semibold text-gray-700 mb-5 uppercase tracking-wide">Items Ordered</h2>
          ...
          {order.items.map((item) => (
            <div key={item.id} className="flex gap-4 items-center">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-0.5">Qty: {item.quantity} × ₪{Number(item.unitPrice).toFixed(0)}</p>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5">
          ...
          {order.trackingNumber && (
            <p className="text-xs text-gray-500 mt-2">Tracking: {order.trackingNumber}</p>
          )}
        </div>
      </div>
