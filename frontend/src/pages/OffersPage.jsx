import { useEffect, useState } from 'react'
import { Link } from 'react-router-dom'
import { getOffers } from '../api/productApi'
import { useLanguage } from '../context/LanguageContext'
import ProductCard from '../components/product/ProductCard'
import Spinner from '../components/ui/Spinner'

/**
 * Offers / discounts feed for customers.
 *
 *  - Loads from GET /api/products/offers (already sorted by largest discount
 *    first on the backend).
 *  - Renders the existing ProductCard, which already shows the original
 *    price (struck through), the discounted price, and the -X% badge.
 *  - Empty state when there are no active offers.
 */
export default function OffersPage() {
  const { t } = useLanguage()
  const [offers, setOffers] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let cancelled = false
    setLoading(true)
    getOffers()
      .then(res => { if (!cancelled) setOffers(res.data.data || []) })
      .catch(() => { if (!cancelled) setOffers([]) })
      .finally(() => { if (!cancelled) setLoading(false) })
    return () => { cancelled = true }
  }, [])

  return (
    <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-12">

      {/* ── Header ──────────────────────────────────────────── */}
      <div className="text-center mb-10">
        <p className="text-[10px] tracking-[0.3em] uppercase text-[#DFA3AD] mb-2"
           style={{ fontFamily: 'Raleway, sans-serif' }}>
          ✦ {t('offers.eyebrow')}
        </p>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-light text-[#3D1A1E]"
            style={{ fontFamily: 'Cormorant Garamond, serif' }}>
          {t('offers.title')}
        </h1>
        <div className="h-0.5 w-16 mt-3 mx-auto"
             style={{ background: 'linear-gradient(90deg, transparent, #DFA3AD, transparent)' }} />
        {!loading && offers.length > 0 && (
          <p className="text-xs text-[#9B7B80] mt-3 tracking-wider">
            {offers.length} {offers.length === 1 ? t('cart.item') : t('cart.items')}
          </p>
        )}
      </div>

      {/* ── Body ────────────────────────────────────────────── */}
      {loading ? (
        <div className="flex justify-center py-20">
          <Spinner size="lg" />
        </div>
      ) : offers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <div className="w-20 h-20 rounded-full bg-[#F9EEF0] flex items-center justify-center mb-6 animate-fade-in-scale">
            <svg className="w-9 h-9 text-[#DFA3AD]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.4}>
              <path strokeLinecap="round" strokeLinejoin="round"
                d="M7 7h.01M7 3h5a1.99 1.99 0 011.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.99 1.99 0 013 12V7a4 4 0 014-4z" />
            </svg>
          </div>
          <h2 className="text-2xl font-light text-[#3D1A1E] mb-2 animate-fade-in-up"
              style={{ fontFamily: 'Cormorant Garamond, serif' }}>
            {t('offers.empty')}
          </h2>
          <p className="text-sm text-[#9B7B80] mb-8 animate-fade-in-up delay-100 tracking-wide max-w-xs">
            {t('offers.emptySub')}
          </p>
          <Link
            to="/products"
            className="animate-fade-in-up delay-200 inline-flex items-center gap-2 px-7 py-3.5 bg-[#6B1F2A] text-white text-xs font-semibold tracking-[0.15em] uppercase rounded-xl hover:bg-[#7D2432] transition-all shadow-sm shadow-[#6B1F2A]/25 hover:shadow-md hover:shadow-[#6B1F2A]/30"
          >
            {t('home.shopNow')}
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
          {offers.map((product, idx) => (
            <div
              key={product.id}
              className="animate-fade-in-up"
              style={{ animationDelay: `${Math.min(idx * 50, 400)}ms` }}
            >
              <ProductCard product={product} />
            </div>
          ))}
        </div>
      )}
    </div>
  )
}
